...
def train_milca12(train_samples, test_samples, train_labels, test_labels, g_train_pos, g_train_neg, p_cutoff, flag):
    ...
    beta1, beta2 = 1, 1
    if flag=='C1':
        if len(g_train_pos) < len(g_train_neg):
            beta1 = 0
        else:
            beta2 = 0

    # On the training set, compute the combined difference as a function of beta
    for beta in betas:
        train_predicted_scores = beta1 * train_predicted_scores_pos + beta2 * beta * train_predicted_scores_neg
        auc_train = roc_auc_score(train_labels, train_predicted_scores)
        train_aucs.append(auc_train)

    # Find the best beta on training set and apply to test set
    ...
    # Compute AUC and accuracy, must replace accuracy with the best accuracy.
    ...
    return auc_test, accuracy_test, best_beta