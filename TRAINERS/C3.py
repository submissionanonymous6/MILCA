...
from UTILS.utils_for_c_tests import opt_threshold_acc
...
class MILCA3(nn.Module):
    ...
    def forward(self, x):
        alphas = F.tanh(self.mu)
        x = self.layer1(self.dropout(alphas * x.float()))
        x = self.layer2(self.dropout(x))
        return F.sigmoid(torch.sum(x, dim=1, keepdim=True)).squeeze(1)
