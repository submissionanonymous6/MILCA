...
def create_NF_figure():
    ...
    for nf in tqdm(num_features):
        ...
        for _ in range(10):
            ...
            train_samples, test_samples, train_labels, test_labels = train_test_split(data, labels, test_size=0.2,
                                                                                      random_state=42)
            auc, acc = C12(train_samples, test_samples, train_labels, test_labels, datasetA, datasetB, 0.05, 'C1')
            current_auc_C1.append(C1_auc)
            auc, acc = C12(train_samples, test_samples, train_labels, test_labels, datasetA, datasetB, 0.05, 'C2')
            current_auc_C2.append(C2_auc)
    ...
    plt.tight_layout()
    plt.savefig("../OUTPUTS/AUC_vs_feat_num_plot.pdf", format="pdf")
    plt.show()