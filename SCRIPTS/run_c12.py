...
from TRAINERS.C12 import train_milca12, C12_new
...
def train_C12(method):
    ...
    output_file = f'../OUTPUTS/new_MILCA_{method}_Test_Results.txt'
        ...
        for i, p_cutoff in enumerate([0.005, 0.01]):
            ...
            for rs in range(95, 105):
                train_samples, test_samples, train_labels, test_labels, g_train_pos, g_train_neg, g_test_pos, g_test_negs = get_train_test_data(
                    dataset_name, rs)
                if method == 'C1':
                    auc, acc, beta = train_milca1(train_samples, test_samples, train_labels, test_labels, g_train_pos, g_train_neg,
                                     p_cutoff=p_cutoff)
                elif method == 'C2':
                    auc, acc, beta = train_milca2(train_samples, test_samples, train_labels, test_labels, g_train_pos, g_train_neg,
                                     p_cutoff=p_cutoff)
